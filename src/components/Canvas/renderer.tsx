...
const DrawLine: React.FC<LineProps> = React.memo(({ x1, y1, x2, y2, color, width, opacity, drag, handleDragEnd }) => {
  ...
  return <Line points={[x1, y1, x2, y2]} opacity={opacity} stroke={color} strokeWidth={width} strokeLinecap="round" draggable={drag} onDragEnd={handleDragEnd} lineCap='round' lineJoin='round'/>;
}, (prevProps, nextProps) => {
  return (
...
const DrawShape: React.FC<ShapeProps> = React.memo(({ x1, y1, type, width, height, radius, color, strokeWidth, fill, drag,handleDragEnd }) => {
  ...
  if (type === 'rectangle' || type === 'square') {
    return <Rect x={x1} y={y1} width={width} height={height} stroke={color} strokeWidth={strokeWidth} draggable={drag} hitStrokeWidth={1} fill={fill} cornerRadius={10}/>;
  } else if (type === 'circle') {
