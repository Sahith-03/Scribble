...
interface toolBarProps{
  ...
  polygonStack: PolygonData[][];
  setPolygonStack: React.Dispatch<React.SetStateAction<PolygonData[][]>>;
}
...
const Canvas: React.FC<toolBarProps> = ({stylusColor,lineWidth,isPanning,setIsPanning,tool,background,polygons,setPolygon,font,fill,fontSize,stageRef,history,setHistory,polygonStack,setPolygonStack}) => {
  ...
  const handleMouseMove = (e: KonvaEventObject<MouseEvent>) => {
    ...
    if (tool === 'rectangle') {
      const width = Math.abs(x - startPoint.x1), height = Math.abs(y - startPoint.y1);
