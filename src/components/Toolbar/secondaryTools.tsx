  ...
  return (
    <div id="secondary-tools" className="fixed flex flex-col top-0 right-0 m-4 p-4 bg-white border border-gray-300 rounded shadow-lg space-y-2 z-10">
        {!["laser","pan","eraser"].includes(tool) && <div className="flex flex-col ">
          ...
          className="w-8 h-8 border rounded-full cursor-pointer focus:outline-none"
          />
        </div>}        
      ...
      {["circle","square","rectangle"].includes(tool) &&
        <div className='flex flex-col'>
          ...
          <div className='flex gap-5'>
            ...
            <input type="button" onClick={(e)=>{setFill("awfawf")}} id="transparent" name="fill"/>
          </div>
        </div>
      }
      <button className="bg-blue-500 text-white rounded p-2 hover:bg-blue-600 transition-colors" onClick={() => {
        setLineWidth(2)
        setFontSize(20)
        setFont(nunito)
      }}>Default</button>
